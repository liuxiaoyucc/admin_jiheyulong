const User = {
    remove(ids, cb) {
        handleAjax('/admin/user/remove', {ids}, 'POST').done((res)=>{
    ...
    create(data, cb) {
        handleAjax('/admin/user/create', data, 'POST').done((res)=>{
    ...
    edit(data, cb) {
        handleAjax('/admin/user/update', data, 'POST').done((res)=>{
            tcr_table(res.message, 1, 1);
            cb && cb(res);
        }).fail(function(msg){
            layer.msg(msg);
    ...
    info(user_id, cb) {
        handleAjax('/admin/user/info', {user_id}).done((res)=>{
            cb && cb(res);
        }).fail(function(msg){
            layer.msg(msg);
        });
